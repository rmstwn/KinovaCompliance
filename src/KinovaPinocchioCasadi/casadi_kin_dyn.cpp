...
namespace casadi_kin_dyn
{
    ...
    std::vector<double> CasadiKinDyn::Impl::ComputeCLIK(std::vector<double> target_position)
    {
        ...
        for (int i = 0; i < max_iterations; ++i)
        {
            ...
            Eigen::MatrixXd I = Eigen::MatrixXd::Identity(J_pos.cols(), J_pos.cols());
            Eigen::MatrixXd J_damped_pinv = J_pos.transpose() * (J_pos * J_pos.transpose() + lambda * lambda * I).inverse();
